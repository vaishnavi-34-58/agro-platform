...
export default function WarehouseManagement() {
  ...
  const load = () => api.get('/admin/warehouses').then(r => { setWarehouses(r.data); setLoading(false); }).catch(() => setLoading(false));
  useEffect(() => { load(); }, []);
  ...
  const handleAddInventory = async (e) => {
    ...
    try {
      ...
      load();
      // Keep selected inventory but we need to fetch it to update it directly, 
      // or just set selectedInventory to null to force refresh.
          ...
          {selectedInventory ? (
            <div className="glass-card p-6 sticky top-0">
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setShowInvModal(true)} className="btn-secondary btn-sm flex items-center gap-1"><Plus size={14} /> Add</button>
              </div>
